...
function InitMesh(program, url)
{
	...
	function(response)
	{
		console.log("Downloaded mesh");
		mesh.Init(program, JSON.parse(response));
	},
	function(error)
	{
		alert('Fatal Error with getting mesh');
		console.error(error);
	});
...
{	
	...
	var cubeMesh = InitMesh(defaultProgram.program, "json/cube.json");
	var cubeTexture = CreateTexture("img/cobble.png");
